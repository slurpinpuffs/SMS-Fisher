...
print(f"Hi! Welcome to the Catalog to Wholesale tool ver. {version}, an SMS Fisher tool by Amani Medcroft.")
...
fileName = splitFile[0] + '_Wholesale'
...
catSheet.cell(row=2, column=2+offset).value = 'Quantity'
...
for num in range(0, maxRow - 1):
    ...
    catSheet.cell(row=catRow, column=2+offset).value = 0
    catSheet.cell(row=catRow, column=2+offset).font = Font(color="000000")
...
i = 1
while i < 16:
    catSheet.delete_cols(12+offset)
    i += 1

catSheet.delete_cols(5+offset)
catSheet.delete_cols(4+offset)
catSheet.delete_cols(3+offset)

if offset == 0:
    col1 = catSheet.column_dimensions['I']
    col2 = catSheet.column_dimensions['J']
elif offset == 1:
    col1 = catSheet.column_dimensions['J']
    col2 = catSheet.column_dimensions['K']
col1.number_format = u'$#,##0.00'
col2.number_format = u'$#,##0.00'

catSheet.cell(row=(catRow + 1), column=3+offset).value = 'Total Value: '
catSheet.cell(row=(catRow + 1), column=4+offset).value = '=SUMPRODUCT(B:B,I:I)'
catSheet.cell(row=(catRow + 2), column=3+offset).value = 'Total Discounted: '
catSheet.cell(row=(catRow + 2), column=4+offset).value = '=SUMPRODUCT(B:B,J:J)'
